...
mongoose.connect("mongodb+srv://kartik:kartik%40123@cluster0.urutz6v.mongodb.net/", {
  ...
  console.log("conneceded to mongodb ")
}).catch((err) => {
...
// endpoints for registration user
app.post('/register', (req, resp) => {
  const { name, email, password, image } = req.body;
  // create a new user object
  // resp.send("hellow im from post")
  ...
  newUser.save().then(() => {
    ...
    console.log('Its KSK Error registering user ', err);
...
const createToken = (userId) => {
  ...
  const token = jwt.sign(payload, "Q$Q$r2K6W8n!jCW%Zk", { expiresIn: "1h" });

  return token;
}

// Endpoints to Login for perticuler users
...
app.post("/friend-request", async (req, res) => {
  ...
  console.warn(`This is current userID ${currentUserId} and this is selected userID ${selectedUserId}`);
